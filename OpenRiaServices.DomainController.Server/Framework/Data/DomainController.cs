...
{
    /// <summary>
    /// Base class for all <see cref="DomainController"/>s.
    /// </summary>
    public abstract class DomainController : ApiController
    {
        ...
        private static IDomainControllerFactory DomainControllerFactory;
        private static MethodInfo countMethod = typeof(DomainController).GetMethod("Count", BindingFlags.NonPublic | BindingFlags.Instance);
        private static MethodInfo enumerateMethod = typeof(DomainController).GetMethod("Enumerate", BindingFlags.NonPublic | BindingFlags.Static);
        ...
        private DomainControllerContext _serviceContext;
        private DomainControllerDescription _serviceDescription;
        ...
        /// <summary>
        /// Gets or sets the <see cref="IDomainControllerFactory"/> used to create new <see cref="DomainController"/> instances.
        /// </summary>
        public static IDomainControllerFactory Factory
        {
            get
            {
                if (DomainController.DomainControllerFactory == null)
                {
                    DomainController.DomainControllerFactory = new DefaultDomainControllerFactory();
                }

                return DomainController.DomainControllerFactory;
            }
            set
            {
                DomainController.DomainControllerFactory = value;
            }
        }
        ...
        /// </summary>
        protected DomainControllerDescription ServiceDescription
        {
            get
            {
                if (this._serviceDescription == null)
                {
                    this._serviceDescription = DomainControllerDescription.GetDescription(this.GetType());
                }
        ...
        /// <summary>
        /// Gets the active <see cref="DomainControllerContext"/> for this <see cref="DomainController"/>.
        /// </summary>
        protected DomainControllerContext ServiceContext
        {
            get
            {
        ...
        /// for all validation operations invoked by the <see cref="DomainController"/>.
        /// </summary>
        /// <value>
        ...
        public AuthorizationResult IsAuthorized(DomainOperationEntry domainOperationEntry, object entity)
        {
                ...
                : new AuthorizationContext(entity, operation, operationType, contextTemplate)))
            {
                ...
                // The principal is retrieved through the DomainControllerContext as a service
                ...
                result = DomainController.EvaluateAuthorization(typeLevelAttributes, principal, context);
                if (result == AuthorizationResult.Allowed)
                {
                    result = DomainController.EvaluateAuthorization(methodLevelAttributes, principal, context);
                }
            }
            return result;
        }
        ...
        /// <summary>
        /// Dispose this <see cref="DomainController"/>.
        /// </summary>
        public void Dispose()
        {
            this.Dispose(true);
        ...
        /// <summary>
        /// Initializes this <see cref="DomainController"/>. <see cref="DomainController.Initialize"/> must be called 
        /// prior to invoking any operations on the <see cref="DomainController"/> instance.
        /// </summary>
        /// <param name="context">The <see cref="DomainControllerContext"/> for this <see cref="DomainController"/>
        /// instance. Overrides must call the base method.</param>
        public virtual void Initialize(DomainControllerContext context)
        {
            if (context == null)
            {
            ...
            {
                throw new InvalidOperationException(Resource.DomainController_AlreadyInitialized);
            }
        ...
        public virtual IEnumerable Query(QueryDescription queryDescription, out IEnumerable<ValidationResult> validationErrors, out int totalCount)
        {
            ...
            try
            {
                ...
                if (!this.ValidateMethodCall(queryDescription.Method, parameters, errors))
                {
                    ...
                    totalCount = DomainController.TotalCountUndefined;
                ...
                try
                {
                    ...
                    {
                        Exception e = DomainController.GetUnwrappedException(tie);
                ...
                if (enumerableResult != null)
                {
                    ...
                    if (queryDescription.Query != null)
                    {
                        ...
                        if (totalCount == DomainController.TotalCountUndefined)
                        {
                            totalCount = this.GetTotalCountForQuery(queryDescription, (IQueryable)enumerableResult, /* skipPagingCheck */ false);
                        }
                    }
                    else if (totalCount == DomainController.TotalCountUndefined)
                    {
                        // Assume the result-set's number of rows is the total count.
                        totalCount = DomainController.TotalCountEqualsResultSetCount;
                    }
                    ...
                    {
                        if (totalCount == DomainController.TotalCountEqualsResultSetCount)
                        {
                    ...
                    // Enumerate the query.
                    Func<IEnumerable, int, IEnumerable> enumerateMethod = DomainController.enumerateMethodMap.GetOrAdd(queryDescription.Method.AssociatedType, type =>
                    {
                        MethodInfo concreteEnumerateMethod = DomainController.enumerateMethod.MakeGenericMethod(type);
                    ...
                    enumerableResult = enumerateMethod(enumerableResult, DomainController.DefaultEstimatedQueryResultCount);
                    if (totalCount == DomainController.TotalCountEqualsResultSetCount)
                    {
            ...
            catch (Exception e)
            {
                ...
                if (tie != null)
                {
                    exceptionToReport = DomainController.GetUnwrappedException(tie);
                }

                DomainControllerErrorInfo error = new DomainControllerErrorInfo(exceptionToReport);
                this.OnError(error);
        ...
        /// <param name="DomainControllerDescription">The <see cref="DomainControllerDescription"/> for the operation being validated.</param>
        ...
        internal static bool ValidateOperations(IEnumerable<ChangeSetEntry> operations, DomainControllerDescription DomainControllerDescription, ValidationContext validationContextRoot)
        {
            bool success = true;
            ...
            foreach (ChangeSetEntry operation in operationsToValidate)
            {
                ...
                if (hasCustomMethod)
                {
                    ...
                    DomainOperationEntry customMethodOperation = DomainControllerDescription.GetCustomMethod(operation.Entity.GetType(), action.Key);
                    if (customMethodOperation.RequiresValidation)
                    {
                        object[] parameters = DomainController.GetCustomMethodParams(customMethodOperation, operation.Entity, action.Value);
        ...
        /// <summary>
        /// Gets the result count for the specified <see cref="IQueryable&lt;T&gt;" />. <see cref="DomainController" />s should 
        ...
        {
            return DomainController.TotalCountUndefined;
        }

        /// <summary>
        /// Disposes this <see cref="DomainController"/>.
        /// </summary>
        ...
        /// the processing of a <see cref="DomainController"/> operation.
        ...
        protected virtual void OnError(DomainControllerErrorInfo errorInfo) { }
                ...
                if (changeSetEntry.EntityActions != null && changeSetEntry.EntityActions.Any())
                {
                    ...
                    if (customMethodOperation == null)
                    {
                        ...
                        string.Format(
                            ...
                            Resource.DomainController_InvalidDomainOperationEntry,
                            entityAction.Key,
                            entityType.Name));
                    }
        ...
        private void CheckOperationType(DomainOperationType operationType)
        {
            ...
            {
                throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture, Resource.DomainController_InvalidOperationType, this.ServiceContext.OperationType, operationType));
            }
        }
        ...
        {
            int totalCount = DomainController.TotalCountUndefined;
        ...
        /// <summary>
        /// Ensures the <see cref="DomainController"/> has been initialized properly.
        /// </summary>
        ...
        private void EnsureInitialized()
        {
            if (this._serviceContext == null)
            {
                throw new InvalidOperationException(Resource.DomainController_NotInitialized);
            }
        }
        ...
        private void InvokeCustomOperations()
        {
            foreach (ChangeSetEntry operation in this.ChangeSet.ChangeSetEntries.Where(op => op.EntityActions != null && op.EntityActions.Any()))
            {
                ...
                object[] parameters = DomainController.GetCustomMethodParams(customMethodOperation, operation.Entity, entityAction.Value);
        ...
        private bool ValidateMethodCall(DomainOperationEntry domainOperationEntry, object[] parameters, List<ValidationResult> validationResults)
        {
            ...
            DomainControllerDescription desc = this.ServiceDescription;
        ...
        /// <summary>
        /// Default <see cref="IDomainControllerFactory"/> implementation.
        /// </summary>
        private class DefaultDomainControllerFactory : IDomainControllerFactory
        {
            /// <summary>
            ...
            public DomainController CreateDomainController(Type DomainControllerType, DomainControllerContext context)
            {
                if (!typeof(DomainController).IsAssignableFrom(DomainControllerType))
                {
                    throw new ArgumentException(
                        string.Format(CultureInfo.CurrentCulture, Resource.DomainController_Factory_InvalidDomainControllerType, DomainControllerType),
                ...
                try
                {
                    DomainController = (DomainController)Activator.CreateInstance(DomainControllerType);
                }
                catch (TargetInvocationException ex)
                {
                    if (ex.InnerException != null)
                ...
                DomainController.Initialize(context);
            ...
            /// </summary>
            /// <param name="DomainController">A <see cref="DomainController"/> instance to release.</param>
            public void ReleaseDomainController(DomainController DomainController)
            {
