...
namespace OpenRiaServices.DomainController.Server
{
    ...
    /// </summary>
    internal class ReflectionDomainControllerDescriptionProvider : DomainControllerDescriptionProvider
    {
        ...
        public ReflectionDomainControllerDescriptionProvider(Type DomainControllerType)
            : base(DomainControllerType, null)
        {
        }

        public override DomainControllerDescription GetDescription()
        {
            DomainControllerDescription description = base.GetDescription();

            // get all type-level attributes
            description.Attributes = ReflectionDomainControllerDescriptionProvider.GetDomainControllerTypeAttributes(description.DomainControllerType);
            ...
            IEnumerable<MethodInfo> methodsToInspect =
                description.DomainControllerType.GetMethods(BindingFlags.Instance | BindingFlags.Public)
            ...
            foreach (MethodInfo method in methodsToInspect)
            {
                ...
                ReflectionDomainOperationEntry operation = new ReflectionDomainOperationEntry(description.DomainControllerType, method, (DomainOperation)(-1));
        ...
        private static void RegisterAssociatedMetadataProvider(MethodInfo methodInfo)
        {
            ...
            {
                ReflectionDomainControllerDescriptionProvider.RegisterAssociatedMetadataTypeTypeDescriptor(type);
            }
        ...
        private static void RegisterAssociatedMetadataTypeTypeDescriptor(Type type)
        {
            ...
            DomainControllerDescription.RegisterCustomTypeDescriptor(new AssociatedMetadataTypeTypeDescriptionProvider(type), type);
        }
        ...
        /// <summary>
        /// This method returns a collection of attributes representing attributes on the <see cref="DomainController"/> <see cref="Type"/>.
        /// </summary>
        /// <param name="DomainControllerType">The <see cref="DomainController"/> <see cref="Type"/>.</param>
        ...
        private static AttributeCollection GetDomainControllerTypeAttributes(Type DomainControllerType)
        {
            // Get attributes from TypeDescriptor
            List<Attribute> attributes = new List<Attribute>(DomainControllerType.Attributes().Cast<Attribute>());

            // All evaluate attributes that exist in interface implementations.
            Type[] interfaces = DomainControllerType.GetInterfaces();
            foreach (Type interfaceType in interfaces)
            {
                // Merge attributes together
                ReflectionDomainControllerDescriptionProvider.MergeAttributes(
                    attributes,
        ...
        /// </summary>
        /// <param name="typeAttributes">DomainController type-level attributes.</param>
        ...
        internal class ReflectionDomainOperationEntry : DomainOperationEntry
        {
            ...
            private Func<DomainController, object[], object> _method;

            /// <summary>
            /// Creates an instance of a <see cref="ReflectionDomainOperationEntry"/>.
            /// </summary>
            /// <param name="DomainControllerType">The DomainController Type the method belongs to.</param>
            /// <param name="methodInfo">The MethodInfo of the method.</param>
            /// <param name="operation">The operation.</param>         
            public ReflectionDomainOperationEntry(Type DomainControllerType, MethodInfo methodInfo, DomainOperation operation)
                : base(DomainControllerType, methodInfo.Name, operation, methodInfo.ReturnType, GetMethodParameters(methodInfo), GetAttributeCollection(methodInfo))
            {
                if (methodInfo == null)
                {
                ...
                // Generic methods aren’t supported, and will be caught during DomainControllerDescription validation.
            ...
            /// </summary>
            /// <param name="DomainController">The <see cref="DomainController"/> instance the operation is being invoked on.</param>
            /// <param name="parameters">The parameters to pass to the method.</param>
            /// <returns>The return value of the invoked method.</returns>
            public override object Invoke(DomainController DomainController, object[] parameters)
            {
                return this._method(DomainController, parameters);
            }
