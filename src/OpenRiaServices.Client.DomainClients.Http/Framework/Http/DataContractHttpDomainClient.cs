...
namespace OpenRiaServices.Client.DomainClients.Http
{
    ...
    abstract class DataContractHttpDomainClient : DomainClient
    {
        ...
        /// <summary>
        /// Initializes a new instance using the provided <see cref="HttpClient"/> and ensures a cached
        /// DataContractSerializationHelper exists for the specified service interface.
        /// </summary>
        /// <param name="httpClient">The <see cref="HttpClient"/> used to send requests.</param>
        /// <param name="serviceInterface">The service interface <see cref="Type"/> for which to obtain or create a serialization helper.</param>
        ...
        /// <summary>
/// Creates an XmlDictionaryWriter for writing a request payload to the provided stream.
/// </summary>
/// <param name="stream">The stream that the writer will write XML to.</param>
/// <returns>An XmlDictionaryWriter configured to serialize XML into the given stream.</returns>
private protected abstract System.Xml.XmlDictionaryWriter CreateWriter(Stream stream);
        /// <summary>
/// Create an XmlDictionaryReader for reading XML from the provided stream.
/// </summary>
/// <param name="stream">The stream containing the XML payload to be read; reader should read from the stream's current position.</param>
/// <returns>An XmlDictionaryReader configured to read the stream and suitable for DataContract deserialization, positioned at the start of the XML content.</returns>
private protected abstract System.Xml.XmlDictionaryReader CreateReader(Stream stream);
        ...
        /// <summary>
        /// Invokes a domain operation on the server and returns its result along with any validation errors.
        /// </summary>
        /// <param name="invokeArgs">Invocation details including operation name, parameters, and expected return type.</param>
        /// <param name="cancellationToken">Token to cancel the operation.</param>
        /// <returns>An InvokeCompletedResult containing the operation's return value (or null) and any collected validation errors.</returns>
        /// <exception cref="System.Exception">Thrown when the server returns a fault that is not a validation error; the fault is mapped to a client exception (for example, DomainOperationException or DomainException).</exception>
        ...
        /// <summary>
        /// Submits the provided change set to the server and returns the server's submit result.
        /// </summary>
        /// <param name="changeSet">The change set to submit.</param>
        /// <param name="cancellationToken">A token to observe while waiting for the operation to complete.</param>
        /// <returns>A <see cref="SubmitCompletedResult"/> containing the original change set and the collection of <see cref="ChangeSetEntry"/> returned by the server (empty if the response contains no entries).</returns>
        ...
        /// <summary>
        /// Executes the given EntityQuery against the service and produces a QueryCompletedResult containing the query results and any validation errors reported by the service.
        /// </summary>
        /// <returns>A QueryCompletedResult containing the root result entities, included entities, the total count (or 0 if none), and any validation errors returned by the service.</returns>
        /// <exception cref="DomainOperationException">If the service returns a fault that is not a validation error; maps service fault codes to operation errors.</exception>
        /// <exception cref="DomainException">If the service fault represents a domain exception.</exception>
        ...
        /// <summary>
        /// Chooses GET or POST based on whether the operation has side effects (and GET availability) and starts the corresponding HTTP request.
        /// </summary>
        /// <param name="operationName">The domain operation name to invoke on the server.</param>
        /// <param name="hasSideEffects">If true, forces a POST; otherwise attempts GET and falls back to POST if GET is not used.</param>
        /// <param name="parameters">Dictionary of parameter names to values to include in the request.</param>
        /// <param name="queryOptions">Optional query parts to include with the request.</param>
        /// <param name="cancellationToken">Token to observe for request cancellation.</param>
        /// <returns>An HttpResponseMessage for the executed request.</returns>
        ...
        /// <summary>
        /// Constructs an XML payload for the given operation (including optional query options and parameters), attaches it to an HTTP POST request, and sends it using the configured HttpClient.
        /// </summary>
        /// <param name="operationName">The operation name used as the request URI and as the root XML element for the operation.</param>
        /// <param name="parameters">A map of parameter names to values to include in the request; parameters with null values are serialized as xsi:nil.</param>
        /// <param name="queryOptions">Optional list of query option parts to include inside a <c>QueryOptions</c> element when present; may be null.</param>
        /// <param name="cancellationToken">Token to observe for request cancellation.</param>
        /// <returns>An HttpResponseMessage for the executed POST request.</returns>
        ...
        /// <summary>
        /// Builds a GET URI from the given operation name, parameters, and query options and issues an HTTP GET request; if the query portion exceeds 2048 characters, signals that GET should be skipped so the caller can use POST instead.
        /// </summary>
        /// <param name="operationName">The operation name appended to the request path.</param>
        /// <param name="parameters">Query parameters to include in the URI; entries with null values are omitted.</param>
        /// <param name="queryOptions">Additional query parts serialized as $-prefixed operators.</param>
        /// <param name="cancellationToken">Token to cancel the HTTP request.</param>
        /// <returns>The HTTP response message for the GET request, or null via a sentinel task if the GET was not attempted because the query portion exceeded 2048 characters.</returns>
        ...
        /// <summary>
        /// Deserializes an HTTP response for the specified domain operation and returns the operation result.
        /// </summary>
        /// <param name="response">The HTTP response message containing the serialized operation result.</param>
        /// <param name="operationName">The domain operation name used to validate expected XML response element names.</param>
        /// <param name="returnType">The expected CLR type of the result used to obtain the appropriate serializer.</param>
        /// <returns>The deserialized result object for the operation, or `null` if the result element is empty.</returns>
        /// <exception cref="FaultException{DomainServiceFault}">If the response contains a DomainServiceFault fault element.</exception>
        /// <exception cref="DomainOperationException">If the HTTP status code is not successful and the response content type does not match the expected content type (maps status codes to appropriate operation error statuses).</exception>
        ...
        /// <summary>
        /// Verifies the XML reader is positioned on an element named {operationName}{postfix} and throws if it is not.
        /// </summary>
        /// <param name="reader">The XML reader to validate.</param>
        /// <param name="operationName">The operation element base name expected at the current reader position.</param>
        /// <param name="postfix">The expected suffix appended to <paramref name="operationName"/> (for example "Response" or "Result").</param>
        /// <exception cref="DomainOperationException">Thrown when the reader is not at the expected XML element.</exception>
        ...
        /// <summary>
        /// Maps a <see cref="DomainServiceFault"/> to the appropriate client exception.
        /// </summary>
        /// <param name="serviceFault">The fault information returned by the service.</param>
        /// <returns>An <see cref="Exception"/> representing the mapped client-side error:
        /// `DomainException` if <paramref name="serviceFault"/> indicates a domain exception;
        /// otherwise a `DomainOperationException` with status `NotSupported` for error code 400,
        /// `Unauthorized` for error codes 401 or 403, and `ServerError` for any other code.</returns>
        ...
        /// <param name="operationName">Name of the operation.</param>
        /// <summary>
        /// Parses a SOAP Fault element from the XML reader and constructs an appropriate FaultException.
        /// </summary>
        /// <param name="reader">An XmlDictionaryReader positioned at the start of a `<Fault>` element.</param>
        /// <param name="operationName">The name of the operation to associate with the returned FaultException.</param>
        /// <returns>`FaultException<DomainServiceFault>` when a `<Detail>` element contains a `DomainServiceFault`; otherwise a plain `FaultException` containing the parsed fault reason and code.</returns>
        ...
        /// <summary>
            /// Retrieves the cached parameter metadata for the specified service method.
            /// </summary>
            /// <param name="methodName">The name of the service method.</param>
            /// <returns>A MethodParameters object describing the parameters of the specified method.</returns>
        ...
        /// <summary>
            /// Gets a <see cref="DataContractSerializer"/> for the specified CLR type.
            /// </summary>
            /// <param name="type">The CLR <see cref="Type"/> to obtain a serializer for.</param>
            /// <returns>A <see cref="DataContractSerializer"/> capable of serializing instances of the specified <paramref name="type"/>.</returns>
