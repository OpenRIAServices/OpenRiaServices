...
## Applying client queries inside a query method

A composable query method can accept an injected `ClientQuery<TEntity>` parameter. This parameter is not exposed in the generated client API. Call `ApplyTo` before materializing an `IQueryable<TEntity>` to execute client filtering, ordering, and paging in the underlying data provider:

```csharp
[Query]
public async Task<List<Product>> GetProducts(
    ClientQuery<Product> clientQuery,
    CancellationToken cancellationToken)
{
    return await clientQuery.ApplyTo(DbContext.Products)
        .ToListAsync(cancellationToken);
}
```

When the client requests a total count, `ApplyTo` retains the unpaged query so the domain service can evaluate the count. Call `SetTotalCount` when the method calculates the count itself; doing so also permits multiple calls to `ApplyTo`. Multiple calls are otherwise supported when a total count was not requested. A query method cannot combine `ClientQuery<TEntity>` with the legacy `out int totalCount` parameter.
