...
#endif
{
    internal static class ValidationUtilities
    {
        ...
        internal static ValidationContext CreateValidationContext(object instance, ValidationContext parentContext)
        {
            ...
            ValidationContext context = new ValidationContext(instance, parentContext != null ? parentContext.Items : null);
            if (parentContext != null)
            {
                context.InitializeServiceProvider(s => parentContext.GetService(s));
            }
            return context;
        }
        ...
        /// <summary>
        /// Gets the DisplayAttribute that applies to a member, etc.
        /// </summary>
        /// <param name="member">A <see cref="ParameterInfo"/> member to query for <see cref="DisplayAttribute"/>s.</param>
        /// <returns>A <see cref="DisplayAttribute"/> found or <c>null</c> if none is found</returns>
        private static DisplayAttribute GetDisplayAttribute(MemberInfo member)
        {
            return member.GetCustomAttributes(typeof(DisplayAttribute), true).SingleOrDefault() as DisplayAttribute;
        }
#endif
