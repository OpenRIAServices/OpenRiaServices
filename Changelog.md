...
* Added key validation support for simple struct key members in client code generation when the struct is shared with the client, implements `IEquatable<T>`, and has no public properties marked with `[Exclude]`.
